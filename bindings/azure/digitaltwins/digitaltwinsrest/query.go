...
func (client QueryClient) QueryTwins(ctx context.Context, querySpecification QuerySpecification, maxItemsPerPage *int32, traceparent string, tracestate string) (result QueryResult, err error) {
    ...
    if err != nil {
    err = autorest.NewErrorWithError(err, "digitaltwinsrest.QueryClient", "QueryTwins", nil , "Failure preparing request")
    return
    }
        ...
        err = autorest.NewErrorWithError(err, "digitaltwinsrest.QueryClient", "QueryTwins", resp, "Failure sending request")
        return
        }

        result, err = client.QueryTwinsResponder(resp)
        if err != nil {
        err = autorest.NewErrorWithError(err, "digitaltwinsrest.QueryClient", "QueryTwins", resp, "Failure responding to request")
        return
        }
